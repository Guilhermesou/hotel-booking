...
function getGuestIdFromRequest(request: NextRequest): number | null {
  const url = new URL(request.url);
  const segments = url.pathname.split("/");
  const idStr = segments[segments.indexOf("guests") + 1]; // pega o valor entre /guests/[id]
...
export async function GET(request: NextRequest) {
  try {
    const id = getGuestIdFromRequest(request);
  ...
  try {
    ...
    const updatedGuest = await prisma.guest.update({
      ...
      data: {
        ...
        email: email !== undefined ? email : existingGuest.email,
        preferences:
          preferences !== undefined ? preferences : existingGuest.preferences,
        notes: notes !== undefined ? notes : existingGuest.notes,
      },
      include: {
        documents: true,
