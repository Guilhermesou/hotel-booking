...
import { Card, Dropdown, DropdownTrigger, DropdownMenu, DropdownItem, addToast, Chip, Tooltip } from "@heroui/react";
import { BedDouble, BedSingle, MoreVertical } from "lucide-react";

export const RoomCard = ({ room }: { room: Room }) => {
    const statusMap: Record<string, { label: string; variant: 'shadow' | 'solid' | 'flat' | 'faded'; color: 'success' | 'warning' | 'danger' | 'default' }> = {
    ...
    const renderQtzBeds = (room: Room) => {
        ...
        return (
            <div className="gap-2 flex flex-row items-center">
                <p>{room.doubleBeds}</p>
                {hasDoubleBeds && <Tooltip content="Camas de casal"><BedDouble size={16} color="white" /></Tooltip>}
                <p>{room.singleBeds}</p>
                {hasSingleBeds && <Tooltip content="Camas de solteiro"><BedSingle size={16} color="white" /></Tooltip>}
            </div>
        )
    }
    return (
        <Card key={room.id} className="relative p-4 space-y-2">
            <div className="flex items-center justify-between">
                ...
                <Dropdown>
                    <DropdownTrigger>
                        <Button variant="ghost" className="w-3 h-8">
